...
RETURNS TABLE(
  ...
  verified boolean, distance_km double precision
)
LANGUAGE sql STABLE
...
AS $$
  SELECT
    ...
    extensions.ST_Distance(p.location, extensions.ST_SetSRID(extensions.ST_MakePoint(user_lon, user_lat), 4326)::extensions.geography) / 1000.0 AS distance_km
  FROM public.pet_friendly_places p
  WHERE
    p.location IS NOT NULL
    AND extensions.ST_DWithin(p.location, extensions.ST_SetSRID(extensions.ST_MakePoint(user_lon, user_lat), 4326)::extensions.geography, radius_km * 1000)
