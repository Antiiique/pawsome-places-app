...
const isAdminEmail = (email: string | null | undefined) => ADMIN_EMAILS.includes(email?.trim().toLowerCase() ?? "");
...
const PlaceDetailPanel = ({
  ...
  const setSnapState = (s: "half" | "full") => {
    snapRef.current = s;
    setSnap(s);
  };
  ...
  const handleDragEnd = () => {
    ...
    const vel =
      velPrevY.current !== null &&
      velCurrY.current !== null &&
      velPrevT.current !== null &&
      velCurrT.current !== null &&
  ...
  async function submitReview() {
    ...
    const payload = {
      ...
      rating: newRating,
      body: newBody || null,
      visited_with_pet: visitedWithPet,
      photos: allPhotos,
    };
    const { error } = userReview
      ? await supabase
          .from("place_reviews")
          .update({ rating: newRating, body: newBody || null, visited_with_pet: visitedWithPet, photos: allPhotos })
          .eq("id", userReview.id)
          .eq("user_id", user.id)
      : await supabase.from("place_reviews").insert(payload);
    if (error) toast.error("Erreur : " + error.message);
    else {
      toast.success(userReview ? "Avis mis à jour !" : "Avis publié !");
      setPhotoFiles([]);
      await loadReviews();
    }
    setSubmitting(false);
        ...
        {adminEditOpen ? (
          <div className="p-4 space-y-4">
            {publisher ? (
              <button
                onClick={() =>
                  window.dispatchEvent(new CustomEvent("open-user-profile", { detail: { userId: publisher.id } }))
                }
                ...
                <div className="w-8 h-8 rounded-full bg-muted overflow-hidden shrink-0">
                  {publisher.avatar_url ? (
                    ...
                    <span className="w-full h-full flex items-center justify-center text-xs font-bold text-muted-foreground">
                      {publisher.display_name?.[0]?.toUpperCase() ?? "?"}
                    </span>
                  )}
                </div>
                <div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  >
                    ...
                    {!CATS.find((c) => c.value === editForm.category) && editForm.category && (
              ...
              <Input
                ...
                onChange={(e) => setEditForm((f) => ({ ...f, opening_hours: e.target.value }))}
                className="h-8 text-sm"
                  ...
                  return displayRating ? (
                    <div className="flex items-center gap-2">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`w-4 h-4 ${i < Math.round(displayRating) ? "text-amber-400 fill-amber-400" : "text-muted"}`}
                        />
                      ))}
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex">
                        {[...Array(5)].map((_, i) => (
                          <Star
                            key={i}
                            className={`w-4 h-4 ${i < Math.round(avgRating) ? "text-primary fill-primary" : "text-muted"}`}
                          />
                        ))}
                      </div>
                    ...
                    <div className="border border-border rounded-xl p-3 space-y-3 bg-background">
                      ...
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map((star) => (
                          ...
                          >
                            <Star
                              className={`w-6 h-6 transition-colors ${star <= (hoverRating || newRating) ? "text-amber-400 fill-amber-400" : "text-muted"}`}
                            />
                          </button>
                        ))}
                      </div>
                                  ...
                                  const valid = files
                                    .filter((f) => {
                                      if (!ALLOWED_IMAGE_TYPES.includes(f.type)) {
                                        toast.error(`${f.name} : format non supporté (JPEG, PNG, WebP)`);
                                        return false;
                                      }
                      ...
                      <label className="flex items-center gap-2 text-xs cursor-pointer">
                        <input
                          type="checkbox"
                          checked={visitedWithPet}
                          onChange={(e) => setVisitedWithPet(e.target.checked)}
                          className="rounded"
                        />
