...
const PlaceDetailPanel = ({ place, onClose, onBack, isFavorite, onToggleFavorite, onReport, isClosing }: PlaceDetailPanelProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Barre admin catégorie — toujours visible, hors zone scrollable */}
      {isAdmin && (
        <div className="shrink-0 px-4 py-2 border-b border-violet-200 dark:border-violet-800 bg-violet-50 dark:bg-violet-950/30 flex items-center gap-2">
          <Tag className="w-3.5 h-3.5 text-violet-500 shrink-0" />
          <span className="text-xs font-semibold text-violet-600 dark:text-violet-400 shrink-0">Catégorie :</span>
          {categoryEditing ? (
            <select
              autoFocus
              ...
              className="flex-1 h-7 rounded-md border border-violet-400 bg-white dark:bg-violet-950 px-2 text-sm font-semibold text-violet-700 dark:text-violet-300 focus:outline-none focus:ring-2 focus:ring-violet-400"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-md px-2 py-0.5 hover:bg-violet-100 dark:hover:bg-violet-900/40 transition-colors"
            >
              <Badge className={`${categoryBgColors[displayCategory] || "bg-gray-500"} text-white text-xs`}>
