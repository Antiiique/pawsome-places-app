...
import { Heart, X, ChevronLeft, ChevronRight, ChevronUp, ShieldCheck, ShieldAlert, Pencil, Trash2 } from "lucide-react";
...
export default function MarkerPopup({
  ...
  async function toggleVerified() {
    if (!dbId || savingMeta) return;
    setSavingMeta(true);
    const newVal = !localVerified;
    const { error } = await supabase.from("pet_friendly_places").update({ verified: newVal }).eq("id", dbId);
    setSavingMeta(false);
    if (error) { toast.error("Erreur : " + error.message); return; }
  ...
  async function toggleFlagged() {
    ...
    const { error } = await supabase.from("pet_friendly_places").update({ is_flagged: newVal }).eq("id", dbId);
    setSavingMeta(false);
    if (error) { toast.error("Erreur : " + error.message); return; }
  ...
  async function handleAdminDelete() {
    if (!dbId || !window.confirm(`Supprimer "${place.name}" définitivement ?`)) return;
    const { error } = await supabase.from("pet_friendly_places").delete().eq("id", dbId);
    if (error) { toast.error("Erreur : " + error.message); return; }
        ...
        {isAdmin && dbId && (
          <div className="shrink-0 border-b border-violet-200 dark:border-violet-800 bg-violet-50 dark:bg-violet-950/30">
            ...
            <div className="flex items-center gap-1.5 px-4 pb-2">
              ...
              >
                <ShieldCheck className="w-3 h-3" />{localVerified ? "Vérifié" : "À vérifier"}
              </button>
              <button
              ...
              >
                <ShieldAlert className="w-3 h-3" />{localFlagged ? "Signalé ⚠️" : "Non signalé"}
              </button>
